...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`absolute top-0 left-0 right-0 flex items-center px-4 sm:px-6 md:px-10 py-6`}>
        {/* LOGO */}
        <div className="flex-1 flex justify-start relative z-30">
        ...
        {/* Desktop Links */}
        <div className="hidden lg:flex items-center gap-2 bg-white/95 backdrop-blur-md rounded-full pl-8 pr-2 py-2 shadow-lg border border-[#2d3a2a]/5 relative z-30">
          {navLinks.map((link, i) => (
        ...
        <div className="flex-1 flex items-center justify-end text-[#2d3a2a] relative z-50">
          <button
      ...
      <div className={`lg:hidden fixed inset-0 z-40 transition-opacity duration-300 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setMenuOpen(false)}>
